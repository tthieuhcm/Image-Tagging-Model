...
if ~isempty(optional_cache_file) && exist(optional_cache_file,'file')
    ...
    if exist('gt_obj_img_ids','var')
        fprintf(['eval_vid_detection :: loaded cache from 2014, ' ...
                 'recomputing\n']);
    else
        bLoadXML = false;
    end
...
if ~isempty(blacklist_file) && exist(blacklist_file,'file')
    blacklist_img_id = textread(blacklist_file,'%d');
    fprintf('eval_vid_detection :: blacklisted %i image/object pairs\n',length(blacklist_img_id));
else
...
unique_track_ids = unique(obj_track_ids);
if length(unique_track_ids) ~= 1 || unique_track_ids ~= -1
  error('track_id must be -1 for per frame object detection from video.');
end
...
for i=1:length(gt_img_ids)
    ...
    blacklisted_img = find(blacklist_img_id == id);
    ...
    tp = zeros(1,num_obj);
    fp = zeros(1,num_obj);
    if ~isempty(blacklisted_img)
        continue; % just ignore this image
    end
    for j=1:num_obj
        bb = bboxes(:,j);
        ovmax = -inf;
    ...
    for k=1:num_gt_obj
        label = gt_labels(k);
        % remove objects in blacklisted images from consideration as positive examples
        if ~isempty(blacklisted_img)
